...
hselect N*.fits[0] $I "OBJECT='CuAr'" > Arc.lst
! awk '{gsub(/.fits\[0\]/,""); print}' Arc.lst > Arc.txt

hselect N*.fits[0] $I "OBJECT='GCALflat'" > Flat.lst
! awk '{gsub(/.fits\[0\]/,""); print}' Flat.lst > Flat.txt

hselect N*.fits[0] $I "OBJECT='Bias'" > Bias.lst
! awk '{gsub(/.fits\[0\]/,""); print}' Bias.lst > Bias.txt

## Uncomment these if using for science
#hselect N*.fits[0] $I "OBSCLASS='science' && OBSTYPE='OBJECT'" > Sci.lst
#! awk '{gsub(/.fits\[0\]/,""); print}' Sci.lst > Sci.txt

## Uncomment these if using for standard stars 
hselect N*.fits[0] $I "OBSCLASS='partnerCal' && OBSTYPE='OBJECT'" > Sci.lst
! awk '{gsub(/.fits\[0\]/,""); print}' Sci.lst > Sci.txt
